...
React hooks for idempotent mutations — built on [`@idemkit/client`](../client). Part of [idemkit](../../README.md).

```bash
npm install @idemkit/react @idemkit/client
```
...
```tsx
import { useIdempotentMutation } from '@idemkit/react';
...
```tsx
import { IdemkitProvider } from '@idemkit/react';
